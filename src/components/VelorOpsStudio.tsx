...
export function VelorOpsStudio() {
  ...
  return (
    <div id="velorops-studio" className="w-full max-w-6xl mx-auto bg-slate-950 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden font-sans text-slate-100">
      
      {/* Studio Header Mockup */}
      <div className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-slate-950/60 border border-slate-800 px-3 py-1 rounded-full text-xs font-medium text-blue-400">
        ...
        <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-lg border border-slate-800/80">
          <button 
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-1.5 ${activeTab === 'queue' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-1.5 ${activeTab === 'composer' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-1.5 ${activeTab === 'channels' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-1.5 ${activeTab === 'analytics' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
      ...
      <div className="grid lg:grid-cols-12 min-h-[500px]">
        ...
        <div className="lg:col-span-3 bg-slate-900/55 p-6 border-r border-slate-800 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-5">
              <div className="bg-slate-950/40 border border-slate-800/80 rounded-xl p-4">
                <span className="text-slate-500 text-xs block font-medium">Queue Status</span>
              ...
              <div className="bg-slate-950/40 border border-slate-800/80 rounded-xl p-4">
                <span className="text-slate-500 text-xs block font-medium">Post Success Rate</span>
                <span className="text-2xl font-bold text-emerald-400 tracking-tight mt-1 block">99.8%</span>
                <span className="text-[10px] text-slate-500 block mt-1">2,410 total posts scheduled via API</span>
              </div>

              <div className="bg-slate-950/40 border border-slate-800/80 rounded-xl p-4">
                <span className="text-slate-500 text-xs block font-medium">Connected Tokens</span>
          ...
          <div className="mt-8 pt-6 border-t border-slate-800/80">
            <div className="flex items-center gap-3">
              <div className="h-8 w-8 rounded-full bg-blue-600/20 border border-blue-500/40 flex items-center justify-center font-bold text-blue-400 text-xs">
                VO
              </div>
              <div>
                <span className="block text-xs font-semibold text-slate-200">velorops@gmail.com</span>
                <span className="block text-[10px] text-slate-500">Developer License</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Content Panel */}
        <div className="lg:col-span-9 p-6 sm:p-8 bg-slate-950">
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Manage and preview videos scheduled to post via the Facebook Graph API.</p>
                  </div>
                  <button 
                    onClick={() => setActiveTab('composer')}
                    className="self-start sm:self-auto flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium px-3.5 py-2 rounded-lg transition-all"
                  >
                ...
                <div className="grid gap-4">
                  {videos.map(video => (
                    <div 
                      key={video.id}
                      className="bg-slate-900 border border-slate-800 hover:border-slate-700/80 rounded-xl p-4 transition-all flex flex-col md:flex-row gap-4 items-start md:items-center justify-between"
                    >
                      <div className="flex gap-4 items-center flex-1">
                        <div className="relative h-16 w-24 rounded-lg overflow-hidden border border-slate-800 flex-shrink-0 bg-slate-950">
                          <img 
                            src={video.thumbnail} 
                            alt={video.title} 
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-sm font-semibold text-white leading-tight">{video.title}</span>
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                              video.status === 'scheduled' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' :
                          ...
                          <span className="block text-xs text-slate-400 flex items-center gap-1">
                            ...
                            {video.page} <span className="text-slate-600">•</span> {video.scheduledTime}
                          </span>
                        </div>
                      </div>

                      <div className="flex items-center gap-2 w-full md:w-auto justify-end border-t border-slate-800/80 md:border-none pt-3 md:pt-0">
                        <button 
                        ...
                        {video.status === 'scheduled' && (
                          <button 
                            onClick={() => handleSimulatePublish(video.id)}
                            className="px-3 py-1.5 bg-blue-600/20 border border-blue-500/30 hover:bg-blue-600 hover:text-white text-blue-400 rounded-lg text-xs font-medium transition-all"
                          >
                            Post Now
                          </button>
                        )}
                ...
                {/* Queue Detail Drawer Simulation */}
                <AnimatePresence>
                  {selectedVideo && (
                    <motion.div 
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <p className="text-xs text-slate-400">Simulating Facebook feed distribution structure.</p>
                        </div>
                        <button 
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-slate-950 rounded-xl border border-slate-800 p-4">
                          <div className="flex items-center gap-2.5 mb-3">
                            ...
                            <div>
                              <span className="block text-xs font-bold text-slate-200">{selectedVideo.page}</span>
                              <span className="block text-[10px] text-slate-500">Sponsored/Automated • Graph API v19.0</span>
                            </div>
                          </div>
                          <p className="text-xs text-slate-300 leading-relaxed mb-3 whitespace-pre-line">{selectedVideo.caption}</p>
                        ...
                        <div className="space-y-4">
                          <div className="bg-slate-950/60 border border-slate-800 rounded-lg p-3">
                            <span className="block text-[10px] font-mono text-slate-500 uppercase">FB Video Endpoint</span>
                            <span className="block text-xs font-mono text-blue-400 mt-1 overflow-x-auto whitespace-nowrap">
                          ...
                          <div className="bg-slate-950/60 border border-slate-800 rounded-lg p-3 space-y-2">
                            <span className="block text-[10px] font-mono text-slate-500 uppercase">Request Payload</span>
                            <div className="text-[11px] font-mono text-slate-400 space-y-1">
                              <div><span className="text-blue-400">"title":</span> "{selectedVideo.title}"</div>
                              <div><span className="text-blue-400">"description":</span> "{selectedVideo.caption.substring(0, 40)}..."</div>
                              <div><span className="text-blue-400">"scheduled_publish_time":</span> "1783584000"</div>
                              <div><span className="text-blue-400">"published":</span> "false"</div>
                            </div>
                          </div>
                          ...
                          <div className="flex gap-2">
                            {selectedVideo.status === 'scheduled' && (
                              <button 
                                onClick={() => handleSimulatePublish(selectedVideo.id)}
                                className="flex-1 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold transition-all"
                              >
            ...
            {activeTab === 'composer' && (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-400">Generate tags, write copy with Gemini, and schedule directly with zero hassle.</p>
                </div>

                <form onSubmit={handleCreateVideo} className="grid md:grid-cols-12 gap-6">
                  {/* Form Block */}
                  <div className="md:col-span-7 space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 mb-1.5">Facebook Target Page</label>
                      <select 
                        value={composerPage}
                        onChange={(e) => setComposerPage(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-200"
                      >
                        {pages.map(p => (
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 mb-1.5">Video Title / Filename</label>
                      <input 
                        type="text"
                        required
                        value={composerTitle}
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        <label className="block text-xs font-semibold text-slate-400">Post Caption</label>
                        <span className="text-[10px] text-slate-500">{composerCaption.length}/2000 chars</span>
                      </div>
                      <textarea 
                        ...
                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-200 resize-none"
                      />
                    </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3.5">
                      ...
                      <div>
                        <span className="block text-[10px] text-slate-400 mb-2">Select Creative Brand Tone:</span>
                          ...
                          {(['hype', 'professional', 'funny', 'educational'] as const).map(tone => (
                            <button
                              ...
                              className={`py-1.5 rounded text-[10px] font-semibold uppercase tracking-wider transition-all ${
                                composerTone === tone ? 'bg-blue-600 text-white' : 'bg-slate-950 hover:bg-slate-800 text-slate-400'
                              }`}
                            >
                              {tone}
                            </button>
                      ...
                      <button
                        ...
                        className="w-full py-2 bg-slate-950 border border-slate-800/80 hover:bg-slate-800 text-slate-200 rounded-lg text-xs font-semibold flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isGeneratingCaption ? (
                          <>
                    ...
                    <button
                      type="submit"
                      className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5"
                    >
                  ...
                  <div className="md:col-span-5 space-y-4">
                    <span className="block text-xs font-semibold text-slate-400">Live Mockup Preview</span>
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-3">
                        <span className="h-2 w-2 rounded-full bg-blue-500 animate-pulse"></span>
                        <span className="text-[10px] text-slate-400 uppercase tracking-wide">Facebook Feed Standard View</span>
                      </div>
                      ...
                      <div className="space-y-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <span className="block text-[11px] font-bold text-slate-200">{composerPage}</span>
                            <span className="block text-[9px] text-slate-500">Just now • Automated</span>
                          </div>
                        </div>

                        <p className="text-[11px] text-slate-300 leading-normal min-h-[40px] whitespace-pre-line bg-slate-950 p-2.5 rounded-lg border border-slate-800/80">
                          {composerCaption || "Your caption will appear here as written or AI generated..."}
                        </p>

                        <div className="aspect-video rounded-lg bg-slate-950 border border-slate-800/80 flex flex-col items-center justify-center text-slate-500 text-center p-4">
                          <Video className="w-8 h-8 text-slate-700 mb-2" />
                          <span className="text-[10px] font-bold text-slate-400 block truncate w-full px-4">
                            {composerTitle || "cinematic_promo.mp4"}
                          </span>
                          <span className="text-[9px] text-slate-600 block mt-1">Ready to upload</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Keep token permissions active and manage publication targets.</p>
                  </div>
                  <button 
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between">
                          ...
                          <input 
                            ...
                            className="h-4 w-7 rounded-full bg-slate-800 border-slate-700 checked:bg-blue-600 text-blue-600 transition-colors cursor-pointer"
                          />
                        </div>
                        <span className="text-slate-500 text-xs mt-1 block">{page.followers} Facebook Followers</span>
                      </div>

                      <div className="pt-3 border-t border-slate-800/80 space-y-2">
                        <div className="flex justify-between items-center text-[10px]">
                          <span className="text-slate-400 font-medium">Access Token State:</span>
                        ...
                        <div className="flex justify-between items-center text-[10px]">
                          ...
                          <span className="text-slate-300 font-mono">{page.expiry}</span>
                        </div>
                      </div>
                      ...
                      {page.tokenStatus === 'warning' && (
                        <button 
                          ...
                          className="w-full py-1.5 bg-blue-600/10 border border-blue-500/30 text-blue-400 hover:bg-blue-600 hover:text-white rounded-lg text-[10px] font-semibold transition-all mt-2"
                        >
                ...
                <AnimatePresence>
                  {isAddingPage && (
                    <motion.div 
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="bg-slate-900/90 border border-slate-800 rounded-xl p-6 mt-6 flex flex-col items-center text-center space-y-4 max-w-md mx-auto"
                    >
                      ...
                      <div className="bg-slate-950 p-4.5 rounded-lg border border-slate-800 w-full text-left space-y-2.5">
                        <div className="flex items-start gap-2.5">
                          <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" />
                          <span className="text-[11px] text-slate-300"><strong>manage_pages</strong>: List linked accounts</span>
                        </div>
                        <div className="flex items-start gap-2.5">
                          <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" />
                          <span className="text-[11px] text-slate-300"><strong>publish_video</strong>: Direct video uploads</span>
                        </div>
                        <div className="flex items-start gap-2.5">
                          <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" />
                          <span className="text-[11px] text-slate-300"><strong>read_insights</strong>: Performance analytics</span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 w-full pt-2">
                        <button 
                          ...
                          className="flex-1 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-semibold transition-all"
                        >
                          Cancel
                        </button>
                        <button 
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Track view performance and automation optimization scores.</p>
                  </div>
                  
                  <div className="flex bg-slate-900 border border-slate-800 p-0.5 rounded-lg">
                    <button 
                      onClick={() => setAnalyticsRange('7d')}
                      className={`px-3 py-1 rounded-md text-[10px] font-semibold transition-all ${analyticsRange === '7d' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-slate-200'}`}
                    >
                      Last 7 Days
                    </button>
                    <button 
                      onClick={() => setAnalyticsRange('30d')}
                      className={`px-3 py-1 rounded-md text-[10px] font-semibold transition-all ${analyticsRange === '30d' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-slate-200'}`}
                    >
                      Last 30 Days
                    </button>
                  </div>
                ...
                {/* Simulated Chart using Pure SVG and CSS for 100% Stability */}
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                  <span className="block text-xs font-semibold text-slate-400 mb-4">Video Views Over Time</span>
                  ...
                  <div className="h-64 w-full relative">
                    ...
                    <div className="absolute top-1/4 left-2/3 bg-slate-950 border border-slate-800 p-2 rounded-lg text-[10px] space-y-0.5 shadow-xl select-none pointer-events-none">
                      <span className="block text-slate-400 font-semibold">Peak Day (Automated)</span>
                  ...
                  {/* Chart Legend */}
                  <div className="flex justify-between items-center text-[10px] text-slate-500 font-medium mt-4 border-t border-slate-800/80 pt-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                    <span className="block text-[10px] text-slate-500 uppercase tracking-wide">Total Impression</span>
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                    <span className="block text-[10px] text-slate-500 uppercase tracking-wide">Engagement (Avg)</span>
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                    <span className="block text-[10px] text-slate-500 uppercase tracking-wide">Shares Triggered</span>
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                    <span className="block text-[10px] text-slate-500 uppercase tracking-wide">Time Saved (Est.)</span>
                    <span className="block text-lg font-bold text-blue-400 mt-1">18 hrs</span>
                    <span className="text-[9px] text-slate-500 mt-1 block font-medium">This month</span>
                  </div>
                </div>
              </motion.div>
            )}
